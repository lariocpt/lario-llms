...
- One extra trap in the same image: `ghcr.io/ggml-org/llama.cpp:server-rocm` keeps `llama-server` at **`/app/llama-server`** — it is **not on `PATH`**. A llama-swap `cmd:` that says bare `llama-server` fails with `executable file not found in $PATH`; `agent-model.sh`'s `emit_model()` bakes the absolute path into the generated `llama-cpp/agent-config.yaml` (which is why that file is never hand-edited — a fix there would be overwritten by the next `agent-model <name>`).
- And one from the mount, not the image: `/config` is the **`./llama-cpp` directory**, bind-mounted, and `agent-config.yaml` inside it is gitignored. On a fresh clone the file does not exist, so `agent-llm` restart-loops with llama-swap's `config not found` — loud, on purpose (a single-file mount would have had docker silently create an empty *directory* in its place). Fix: `./agent-model.sh config muse-glimmer-dflash` (the default entry since 2026-08-31; it was `config muse-glimmer` for the one day before), which writes the file without touching the container.
- And three from the DFlash drafter, since `muse-glimmer-dflash` became the default (2026-08-31): (a) the **draft KV must stay f16** (`-ctkd f16 -ctvd f16`) — quantizing it collapses acceptance, and the speed-up with it; the *target's* q8_0 KV is a separate setting and is fine. (b) In llama.cpp **DFlash is not output-identical** to plain decoding — a published DGX-Spark recipe measured greedy parity 0/10, whatever the model card says — so treat it as a different sampler, not a free speed-up; the temp-0 A/B on agent-shaped prompts here came out 3 of 4 byte-identical, 4th equivalent. (c) A slot that is decoding while the *other* slot prefills ~120k tokens **starves to ~1 tok/s** until that prefill finishes — llama.cpp interleaves the prefill ubatches; it happens without DFlash too, so don't blame the drafter (or declare the provider dead). Rollback for all three: `agent-model muse-glimmer` (3 slots, no drafter).
